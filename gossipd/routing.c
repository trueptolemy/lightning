			    ...
			    size_t max_hops)
{
	...
	for (size_t i = 0; i < tal_count(excluded); i++) {
		if (excluded[i]->type == EXCLUDE_CHANNEL) {
			struct short_channel_id_dir *chan_id = (struct short_channel_id_dir *)&excluded[i]->u;
			...
			struct node *node = get_node(rstate, (struct node_id *)&excluded[i]->u);
			if (!node)
				continue;
			struct chan_map_iter i;
			struct chan *chan;
			for (chan = first_chan(node, &i); chan; chan = next_chan(node, &i)) {
				...
				struct short_channel_id_dir *id= tal(tmpctx, struct short_channel_id_dir);
				id->scid = chan->scid;
				id->dir = dir;
