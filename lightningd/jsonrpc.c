...
/* The manager of internal rpcmethod calls. */
...
/* This represents a JSON RPC connection.
 * No io_conn, no partial read, no writer and only one command. */
	      ...
	      const jsmntok_t tok[])
{
	...
	/* The timer loop of `lightning` will make the timer callback in
	 * db transaction. */
