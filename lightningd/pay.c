				...
				const char *details)
{
	...
	list_for_each_safe(&ld->waitsendpay_commands, pc, next, list) {
		...
		sendpay_fail(pc->cmd, payment, pay_errcode,
				     onionreply, fail, details);
	}
}
					   ...
					   const struct sha256 *payment_hash)
{
	...
	case PAYMENT_FAILED:
		...
		if (!failonionreply && !failnode) {
			...
			return sendpay_fail(cmd, payment, PAY_UNPARSEABLE_ONION,
					    failonionreply,
					    NULL, faildetail);
		} else {
	     ...
	     const char *b11str TAKES)
{
	...
	if (failcode) {
		...
		return sendpay_fail(cmd, payment, PAY_TRY_OTHER_ROUTE, NULL,
