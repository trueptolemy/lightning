...
struct plugin_request {
	const char *name;
	bool (*response_cb)(void *arg, const char *buffer, const jsmntok_t *toks);
 ...
 */
void plugin_request_call_(struct lightningd *ld, const tal_t *ctx,
...
#define PLUGIN_REQUEST_CALL_DEF(name, payload_type, response_cb_arg_type)         \
	UNNEEDED static inline void plugin_request_call_##name(                   \
	    struct lightningd *ld, const tal_t *ctx,                           \
	    payload_type payload,                                              \
	    response_cb_arg_type cb_arg,                                       \
	    enum request_async_prio prio)                                      \
	{                                                                      \
		plugin_request_call_(ld, ctx, &name##_request_gen, (void *)payload,       \
				     (void *)cb_arg, prio);                             \
	}
...
#define REGISTER_PLUGIN_REQUEST(name, response_cb, response_cb_arg_type,          \
	...
	struct plugin_request name##_request_gen = {                                 \
	    stringify(name),                                                   \
	    typesafe_cb_cast(bool (*)(void *, const char *, const jsmntok_t *),\
			     bool (*)(response_cb_arg_type,		       \
