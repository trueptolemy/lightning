			 ...
			 int exitstatus)
{
	...
	new_reltimer(bitcoind->timers, notleak(bcli), time_from_sec(1),
		     retry_bcli, bcli);
}
...
static bool process_estimatefee(struct bitcoin_cli *bcli)
{
	...
	if (!extract_feerate(bcli, bcli->output, bcli->output_bytes, &feerate)) {
		plugin_log(LOG_UNUSUAL, "Unable to estimate %s/%u fee",
		 ...
		 * all the time. */
		if (bcli->bitcoind->chainparams->testnet)
...
static bool process_sendrawtx(struct bitcoin_cli *bcli)
{
	...
	plugin_log(LOG_DBG, "sendrawtx exit %u, gave %s",
		  *bcli->exitstatus, msg);
...
void bitcoind_sendrawtx_(struct bitcoind *bitcoind,
			 ...
			 void *arg)
{
	plugin_log(LOG_DBG, "sendrawtransaction: %s", hextx);
...
static bool process_gettxout(struct bitcoin_cli *bcli)
{
	...
	if (*bcli->exitstatus != 0 || bcli->output_bytes == 0) {
		plugin_log(LOG_DBG, "%s: not unspent output?",
			  bcli_args(tmpctx, bcli));
...
static bool process_getblock(struct bitcoin_cli *bcli)
{
	...
	*/
	...
	if (!txidtok) {
		plugin_log(LOG_DBG, "%s: no txnum %u",
...
static bool process_getblockhash_for_txout(struct bitcoin_cli *bcli)
{
	...
	if (*bcli->exitstatus != 0) {
		void *cbarg = go->cbarg;
		plugin_log(LOG_DBG, "%s: invalid blocknum?",
			  bcli_args(tmpctx, bcli));
		tal_free(go);
		cb(bcli->bitcoind, NULL, cbarg);
		return true;
