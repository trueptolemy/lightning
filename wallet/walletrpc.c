					      ...
					      struct unreleased_tx **utx)
{
	...
	*utx = tal(cmd, struct unreleased_tx);
	...
	if (!param(cmd, buffer, params,
		   p_req("output", param_array, &outputtok),
	...
	outputs = tal_arr(tmpctx, struct bitcoin_tx_output *, outputtok->size);
	out_len = 0;
	(*utx)->wtx->all_funds = false;
	json_for_each_arr(i, t, outputtok) {
		...
		if (!destination || !amount)
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
					    "outputs[%zi]: must have destination"
					    " and satoshi", i);
		...
		if (amount_sat_eq(*amount, AMOUNT_SAT(-1ULL))) {
			if (outputtok->size > 1)
				return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
						    ...
						    " all satoshi. The amount of outputs"
